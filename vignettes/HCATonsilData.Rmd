...
Harmony [@mereu2020benchmarking]. Harmony scales well to atlas-level datasets
...
TODO: mention HCATonsilDataInfo, describe that the first time the dataset is downloaded might take longer (depending on size of tha dataset), but once it's downloaded HCATonsilAtlas will upload it from cache instead, speededing up things.

TODO: mention color palettes, same as in paper

TODO: mention timestamped annotation. open an issue if needed

TODO: mention how to save .h5ad --> INTEROPERABILITY!!

TODO: only show example with epithelial: needs to be fast!!! version 1.0 and 2.0

TODO: include spatial and Rna=All downloads, but with eval=FALSE
...
As you can see, this object consists of 377,988 profiled with scRNA-seq (3P)
and 84,364 cells profiled with multiome. We can dowload a `SingleCellExperiment`
object specific to each of the main subpopulations defined at level 1 as follows:

```{r}
listCellTypes(assayType = "RNA")
(epithelial <- HCATonsilData(assayType = "RNA", cellType = "epithelial"))
# df <- as.data.frame(colData(epithelial))
# ggplot(df, aes(UMAP_1_20220215, UMAP_2_20220215, col = annotation_20230508)) +
#   geom_point() +
#   scale_color_manual(values = cols) +
#   labs(title = "Annotation 20230508", x = "UMAP1", y = "UMAP2") +
#   theme_classic() +
#   theme(
...
it. If other immunologists look at the data and propose modifications, we will
add additional time-stamped annotations in future releases of `HCATonsilData`.
...
* annotation_20220215, annotation_20220619, annotation_20230508: time-stampled annotation for cell types.
...
Here are the instructions to download the scATAC-seq object:
...
download.file(url = atac_url, destfile = "download_dir/TonsilAtlasSeuratATAC.tar.gz")
# Advice: check that the md5sum is the same as the one in Zenodo
untar("download_dir/TonsilAtlasSeuratATAC.tar.gz", exdir = "download_dir")
atac <- readRDS("download_dir/scATAC-seq/20230911_tonsil_atlas_atac_seurat_obj.rds")
atac
```


The multiome object contains 68,749 cells that passed both RNA and ATAC QC filters.
Here are the instructions to download the Multiome object:
...
download.file(url = multiome_url, destfile = "download_dir/TonsilAtlasSeuratMultiome.tar.gz")
# Advice: check that the md5sum is the same as the one in Zenodo
untar("download_dir/TonsilAtlasSeuratMultiome.tar.gz", exdir = "download_dir")
multiome <- readRDS("download_dir/multiome/20230911_tonsil_atlas_multiome_seurat_obj.rds")
multiome
```
...
as prefix. These files can be downloaded as follows:

```{r eval=FALSE}
dir.create("download_dir")
fragments_url <- "https://zenodo.org/record/8373756/files/fragments_files.tar.gz"
download.file(url = fragments_url, destfile = "download_dir/fragments_files.tar.gz")
untar("download_dir/fragments_files.tar.gz", exdir = "download_dir")
```
...
* TSS.percentile
* RNA.weight: the weight that RNA has in the weighted nearest neihbor graph per cell (multiome-specific)
* ATAC.weight: the weight that RNA has in the weighted nearest neihbor graph per cell (multiome-specific)


## CITE-seq

The CITE-seq object can be downloaded as follows:
...
download.file(url = cite_url, destfile = "download_dir/TonsilAtlasSeuratCITE.tar.gz")
# Advice: check that the md5sum is the same as the one in Zenodo
untar("download_dir/TonsilAtlasSeuratCITE.tar.gz", exdir = "download_dir")
cite <- readRDS("download_dir/CITE-seq/20220215_tonsil_atlas_cite_seurat_obj.rds")
cite
```
...
```{r eval=FALSE}
scirpy_df <- read.csv(
  "download_dir/CITE-seq/scirpy_tcr_output.tsv",
  header = TRUE
)
```
...
```{r eval=FALSE}
(spatial <- HCATonsilData(assayType = "Spatial"))

# TODO Helena: Document basic slots of SpatialExperiment object
```
